...
It seems as if the Android App uses OAuth endpoints implemented in a Xenforo extension, to authenticate users with a token they can use. Interestingly, the API doesn't authenticate users to ensure that they have a token, meaning I can make API requests without one. The Android API wasn't part of the IP ban either, making me believe
