...
GitLab Flavored Markdown supports `Mermaid`/`PlantUML`. GitHub Flavored Markdown added
for this too in March 17, 2022. Both `Mermaid` and `PlantUML` are “Markdownish ways
to render diagrams”. Since Github Flavored Markdown doesn’t support either, I can’t
demonstrate each. Here is some raw text that displays some examples for each:
