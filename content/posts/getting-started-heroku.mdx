...
Here are some recommended prerequisite skills before you get started with Heroku:
...
- #4. This creates a new router that listens for requests on the `/` path. When a
  request is recieved, it returns `"Hello world"{:js}`

- #8. In Heroku, the `PORT{:js}` enviorment variable is set to the port that has to be
  listened to. Over here, we are listening to `1455{:js}` if there is no `PORT{:js}` (development)

- #10. This makes the application actually listen for any requests and logs the port
  listened to
